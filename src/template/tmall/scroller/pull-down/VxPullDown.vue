<template>
  <div class="zz-page vpd-page">
    <!-- title -->
    <div class="vpd-title-container">{{title}}</div>
    <!-- scroll view list -->
    <scroller lock-x scrollbar-y use-pulldown :pulldown-config="pullDownConfig" @on-pulldown-loading="onPullDownEvent"
              v-model="status" ref="scroller" class="vpd-scroll-sty2">
      <div class="box2">
        <div class="vpd-scroll-inner2">
          <div v-for="(item,index) in scrollList" v-bind:key="index" class="vpd-item-container">
            <span class="vpd-item-text">列表内容{{item}}</span>
          </div>
        </div>
      </div>
      <!-- custom pull down slot -->
      <div slot="pulldown" class="xs-plugin-pulldown-container xs-plugin-pulldown-down vpd-pull-down-slot">
